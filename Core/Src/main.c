...
uint8_t Self_Driving_state(void) {
  ...
  if (FL_Dist >= 20 && FR_Dist >= 20) { // 장애물이 충분히멀다
    return FORWARD;
  } else if (FR_Dist < 15 && FL_Dist >= 15) { // 오른쪽 장애물이 가까운 경우
    return LEFT; // 왼쪽으로 회피
  } else if (FL_Dist < 15 && FR_Dist >= 15) { // 왼쪽 장애물이 가까운 경우
    return RIGHT; // 오른쪽으로 회피
  } else if (FL_Dist < 15 && FR_Dist < 15 && B_Dist >= 15) { // 전방에 장애물이 있는 경우 후진
    return BACKWARD;
  } else if (FL_Dist < 2 || FR_Dist < 2) { // 장애물이 매우 가까운 경우 정지
    return BREAKING;
  } else {
    return FORWARD; // 기본적으로 정지 상태 유지
  }
}
